...
h5_filename = 'cutout_stacks_CFISr_PS1i_HSCg_200x200'
SLURM_TMPDIR = os.getenv('SLURM_TMPDIR')

cam_dict = { 'cfis' : {'band': 'r'},
              'ps1' : {'band': 'i'},
              'hsc' : {'band': 'g'}}
...
for tile in tiles: 
    ...
    cfis_id_list = partitioned_data['CFIS_ID'].values
    
    n_sources = len(ra_dec_list)
    print('There are {} sources to cut out'.format(n_sources))
    
    for cam in cam_dict.keys():
        tile_fitsfilename = '{}.{}.{}.fits'.format(cam.upper(), tile, cam_dict[cam]['band'])
        tile_catfilename = '{}.{}.{}.cat'.format(cam.upper(), tile, cam_dict[cam]['band'])
        tile_fitsfilepath = os.path.join(download_dir, cam, tile_fitsfilename)
    ...
    if not os.path.exists(cam_dict['cfis']['tile_filepath']):
        cam_dict['cfis']['tile_filepath'] = '/home/merileo/projects/rrg-kyi/astro/cfis/W3/CFIS.{}.r.fits'.format(tile)
